...
const SCHEDULE = [
  ...
  { age:'9–12 Months',    vaccines:['MR-1 (Measles-Rubella)','JE-1 (endemic areas)','Vitamin A-1'], note:'Important milestone' },
  { age:'16–24 Months',   vaccines:['DPT Booster-1','OPV Booster','MR-2','JE-2 (endemic)'], note:'Booster doses critical' },
  { age:'5–6 Years',      vaccines:['DPT Booster-2'], note:'School entry' },
  { age:'10 Years',       vaccines:['Td (Tetanus-Diphtheria)'], note:'Adolescent booster' },
  { age:'16 Years',       vaccines:['Td Booster'], note:'' },
  { age:'Pregnant Women', vaccines:['Td-1','Td-2 / Td Booster','IFA + Calcium Tablets'], note:'Antenatal care schedule' },
];

// GET /api/vaccination/centres?location=&type=
router.get('/centres', (req, res) => {
  let centres = [...CENTRES];
  if (req.query.type) centres = centres.filter(c => c.type.toLowerCase() === req.query.type.toLowerCase());
  res.json({ success: true, centres, total: centres.length });
});
